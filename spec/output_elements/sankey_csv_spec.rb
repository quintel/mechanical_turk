  ...
  Turk::PresetCollection.from_keys(:ii3050v2,:ip2024).each do |scenario|
    context "with scenario #{scenario.original_scenario_id}" do
      ...
      it "checks the turk_sankey_electricity" do
        ...
        puts "Actual value: #{actual_value}, Expected range: #{expected_value - 10.0} to #{expected_value + 10.0}"
      
        expect(actual_result).to sankey_equal(expected_value) # Pass the entire object if the matcher handles the extraction
      end
